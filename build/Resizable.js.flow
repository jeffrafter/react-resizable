...
export default class Resizable extends React.Component<Props, State> {
  static propTypes = {
    ...
    dir: PropTypes.oneOf(['default', 'invert']),
  ...
  resizeHandler(handlerName: string): Function {
    return (e: SyntheticEvent<> | MouseEvent, {node, deltaX, deltaY}: DragCallbackData) => {
      ...
      const invert = this.props.dir === 'invert' ? -1 : 1;

      // Update w/h
      let width = this.state.width + ((canDragX ? deltaX : 0) * invert);
      let height = this.state.height + ((canDragY ? deltaY : 0) * invert);
  ...
  render(): ReactNode {
    ...
    const {children, draggableOpts, width, height, handleSize,
        lockAspectRatio, axis, dir, minConstraints, maxConstraints, onResize,
